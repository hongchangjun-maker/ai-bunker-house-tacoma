# AI벙커하우스 1호 — 타코마 장수 리조트 구축안

전북특별자치도 장수군 계남면 장수로 2662-11의 기존 관광농원 부지 재생 가능성을 바탕으로 한 공개형 건축·운영 구축안 웹사이트입니다. 여기서 구축은 공간 프로그램과 검증 체계를 뜻하며 현장 시공 완료를 의미하지 않습니다.
...
- 대규모 태양광·미니 풍력·CCTV·외곽 철조망·모래주머니 방호선·로봇/드론 순찰 배치안
- AVIF/WebP 이중 포맷, 640/1280/약 1690px 반응형 변형, 지연 로딩과 1년 정적 캐시
- 주소 기반 OpenStreetMap 임베드 및 외부 지도 링크
- 근거 출처와 확인 필요 사항 분리
- 하나의 13,000㎡ 면적표로 맞춘 지상-지하 6개 층 기본계획 단면도
- B1~B6 인터랙티브 탐색과 고화질 3D 평면도 팝업, 구역별 면적표
- 144침상과 5년 물·열량 계획 수요를 자동 검증하는 운영 기준
- 12개 전문가 위험 등록부, 시민 검증 6문항과 인원·기간·물 수요 가정 계산기
- 안전·성능 검증 완료 0건을 정직하게 표시하고 착수 전 필수 항목을 공개
- 국문/영문 주요 카피 전환
...
이미지 교체 시 저작권, 인물권, 상표권을 확인하고 AI 이미지에는 재구성·계획 이미지 표기를 유지합니다.
